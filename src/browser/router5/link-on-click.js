...
export const shouldInterceptEvent = router => evt => {
    ...
    if (el.hasAttribute('download') || el.getAttribute('rel') === 'external') return false;
...
export const onClick = router => evt => {
    ...
    if (routeMatch) {
        ...
        throw new Error(`[router5 driver] Could not match clicked hyplink href ${evt.target.href} to any known route`);
    }
};
